...
class bookingController {
	...
	async getBookingsRequester(req: Request, res: Response) {
		...
		try {
			const bookings = await bookingService.getBookingByRequesterAndStatus(
				userFind,
				status
			);
	...
	async getBookingsRequested(req: Request, res: Response) {
		...
		try {
			const bookings = await bookingService.getBookingByRequestedAndStatus(
				userFind,
				status
			);
			res.status(200).json(bookings);
	...
	async answerBooking(req: Request, res: Response) {
		...
		try {
			schema.parse(req.body);
			const booking = await bookingService.answerBooking(
				req.body.bookingId,
				req.body.answerBooking
			);
			res.status(200).json(booking);
		} catch (error) {
			if (error instanceof Error)
	...
	async getBookingsWithVisitsByCatSitterId(req: Request, res: Response) {
		...
		try {
			const bookings = await bookingService.getBookingsWithVisitsByCatSitterId(
				catSitterId
			);
			res.status(200).json(bookings);
		} catch (error) {
